...
/*	for(int i=0; i<r; i++){
	...
	for(int i=0; i<r; i++){
		...
		for(int j=0; j<c; j++){
			...
			if(rowMin[i] > matrix[i][j]){
				rowMin[i] = matrix[i][j];
				//cout<<"rowMin["<<i<<"] is "<<rowMin[i]<<endl;
			}
		}
	}
	...
	for(int j=0; j<c; j++){
		...
		for(int i=0; i<r; i++){
			if(colMax[j] < matrix[i][j]){
				colMax[j] = matrix[i][j];
				//cout<<"colMax["<<j<<"] is "<<colMax[j]<<endl;	
			}
		}
	}
	...
	for(int i=0; i<r; i++){
		for(int j=0; j<c; j++){
			if(matrix[i][j] == colMax[j] &&(colMax[j] == rowMin[i])){
			//	cout<<"ans is matrix["<<i<<"]["<<j<<"] is "<<matrix[i][j]<<endl;
				ans = matrix[i][j];
			}
		}
	}
